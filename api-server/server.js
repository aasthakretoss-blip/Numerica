...
// ✅ TEST: Verify server is running latest code
app.get('/api/test-server-version', (req, res) => {
  res.json({
    success: true,
    message: 'Server is running updated code with RFC endpoint',
...
app.post('/api/auth/validate-email', async (req, res) => {
  console.log('📨 Handler /api/auth/validate-email ejecutándose');
  ...
  try {
    ...
    console.log('🔍 Validando email para registro:', email);
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ...
        return res.status(403).json({
          success: false,
          error: 'Este correo electrónico no está autorizado para registrarse. Contacte al administrador.'
        });
      }
      
      const user = result.rows[0];
      
      console.log('📊 Status del usuario en DB:', user.status);
      
      // Solo permitir registro si el status es 'pending_setup'
      if (user.status !== 'pending_setup') {
        console.log('⚠️ Usuario ya registrado o no elegible. Status:', user.status);
        return res.status(409).json({
          success: false,
          error: 'Este correo ya tiene una cuenta registrada. Por favor inicia sesión.'
        });
      }
      ...
      try {
        ...
        // Si llegamos aquí, el usuario ya existe en Cognito
        console.log('⚠️ Usuario ya existe en Cognito pero DB status es pending_setup');
        console.log('🗑️ Eliminando usuario de Cognito para permitir re-registro...');
        ...
        console.log('✅ Usuario eliminado de Cognito, puede proceder con el registro');
        ...
        if (cognitoError.name === 'UserNotFoundException') {
          console.log('✅ Email válido para registro (no existe en Cognito):', email);
...
app.get('/api/payroll/periodos-from-curp', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`📅 VALORES ÚNICOS DE CVEPER (${uniqueCvepers.length} períodos):`);
      ...
      console.log(`3. ✅ Identificados ${uniqueCvepers.length} períodos únicos`);
...
// ✅ FIXED: Obtener RFC basado en CURP desde historico_nominas_gsau
// IMPORTANT: This route MUST be defined BEFORE the general /api/payroll route
// Route registered at: Line 731
// Specification: GET /api/payroll/rfc-from-curp?curp=<CURP_VALUE>
app.get('/api/payroll/rfc-from-curp', async (req, res) => {
  // ✅ VERIFICATION: This log confirms the route is being hit
  console.log('\n🔵🔵🔵 [RFC-FROM-CURP ENDPOINT CALLED] 🔵🔵🔵');
  console.log('🔵 Route registered and accessible!');
  console.log('🔵 Full URL:', req.protocol + '://' + req.get('host') + req.originalUrl);
  console.log('🔵 Query params:', req.query);
  console.log('🔵 CURP received:', req.query.curp);
  
  try {
    const { curp } = req.query;
    
    // ✅ Requirement: Validate curp parameter - return 400 if missing
    if (!curp) {
      console.log('❌ CURP no proporcionado');
      return res.status(400).json({
        success: false,
        message: "Missing 'curp' parameter"
      });
    }
    
    const cleanedCurp = String(curp).trim();
    console.log(`🔍 Buscando RFC para CURP: ${cleanedCurp}`);
    ...
    try {
      ...
      console.log(`📊 Resultado de la query: ${result.rows.length} filas encontradas`);
      
      // ✅ Requirement: Return 404 if not found (matching specification)
      if (result.rows.length === 0) {
        console.log(`⚠️ No se encontró RFC para CURP: ${cleanedCurp}`);
        return res.status(404).json({
          success: false,
          message: `RFC not found for CURP: ${cleanedCurp}`
        });
      }
      
      const rfc = result.rows[0].RFC;
      console.log(`✅ RFC encontrado para CURP ${cleanedCurp}: ${rfc}`);
      
      // ✅ Requirement: Return success with data structure matching specification
      return res.json({
        success: true,
        data: {
          curp: cleanedCurp,
      ...
      console.error('❌ Error en query de base de datos:', dbError);
      throw dbError;
    } finally {
      client.release();
    }
    ...
    // ✅ Requirement: Error handling - return 500 for internal errors
    console.error('❌ Error obteniendo RFC desde CURP:', error);
    console.error('❌ Stack trace:', error.stack);
    return res.status(500).json({
      success: false,
      message: "Server error retrieving RFC from CURP"
    });
  }
});
...
app.get('/api/payroll/name-from-curp', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(`🔍 Ejecutando query para buscar nombre completo de CURP: ${curp}`);
      const result = await client.query(query, [curp]);
      ...
      console.log(`✅ Nombre completo encontrado para CURP ${curp}: ${nombreCompleto}`);
...
app.get('/api/fpl/data-from-rfc', async (req, res) => {
  try {
    ...
    console.log(`🏦 Buscando datos FPL completos para RFC: ${rfc}`, fechaFPL ? `Fecha FPL: ${fechaFPL}` : 'Todas las fechas');
    ...
    try {
      ...
      if (originalFecpla && originalAntiguedad) {
        ...
        console.log(`🔍 Búsqueda específica con datos originales: fecpla=${originalFecpla}, antiguedad=${originalAntiguedad}`);
        ...
        if (antiguedadColumn) {
          ...
          throw new Error('No se pudo identificar la columna de antigüedad para búsqueda específica');
        }
      } else {
...
app.get('/api/payroll/fecpla-from-rfc', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('🔍 Analizando estructura completa de historico_fondos_gsau...');
      const allColumnsQuery = `
        SELECT column_name, data_type
        FROM information_schema.columns 
        WHERE table_name = 'historico_fondos_gsau'
      ...
      for (const exactName of exactNames) {
        ...
        if (found) {
          antiguedadColumn = found.column_name;
          console.log(`✅ Columna encontrada por nombre exacto: "${antiguedadColumn}"`);
          break;
        }
      }
      ...
      if (!antiguedadColumn) {
        ...
        if (keywordMatches.length > 0) {
          ...
          console.log('📋 Otros candidatos:', keywordMatches.map(r => r.column_name));
        }
      }
        ...
        for (const numCol of numericColumns.slice(0, 10)) { // Revisar solo primeros 10
          try {
            ...
            if (stats.positivos > 0 && stats.min_val >= 0 && stats.max_val <= 50 && stats.avg_val <= 15) {
              ...
              console.log(`✅ Columna detectada por análisis de contenido: "${antiguedadColumn}"`);
              console.log(`   📊 Estadísticas: ${stats.positivos} registros, rango: ${stats.min_val}-${stats.max_val}, promedio: ${parseFloat(stats.avg_val).toFixed(4)}`);
              break;
            }
          } catch (e) {
      ...
      if (!antiguedadColumn) {
        console.log('❌ No se pudo identificar la columna de Antigüedad en Fondo');
        return res.json({
          success: false,
      ...
      result.rows.forEach((row, index) => {
        ...
        console.log(`${index + 1}: Base: ${fechaBase}, Antigüedad: ${antiguedadAnos} años, FPL: ${fechaCalculada}`);
      ...
      console.log(`📅 FECHAS FPL ÚNICAS CALCULADAS (${uniqueFecplasFPL.length} fechas):`);
      uniqueFecplasFPL.forEach((item, index) => {
        console.log(`${index + 1}: ${item.fechaCalculada.toISOString().split('T')[0]} (Base: ${item.fechaBase}, +${item.antiguedadAnos} años)`);
      });
      ...
      const formattedDates = uniqueFecplasFPL.map(item => {
        ...
        return {
          ...
          metadata: {
            ...
            calculoAplicado: `${item.fechaBase.toISOString().split('T')[0]} + ${item.antiguedadAnos} años = ${displayValue}`,
      ...
      console.log(`2. ✅ Extraídos ${result.rows.length} registros con fecpla y antiguedad`);
      console.log(`3. ✅ Calculadas fechas FPL: fecpla + (antiguedad_anos * 365.25 días)`);
      console.log(`4. ✅ Identificadas ${uniqueFecplasFPL.length} fechas FPL únicas`);
...
app.get('/api/payroll/filter-options', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Error obteniendo opciones de filtros demográficos:', error);
    res.status(500).json({
      success: false,
      error: error.message,
...
app.get('/api/payroll/filters', async (req, res) => {
  ...
  try {
    ...
    // CORREGIDO: Manejar cveper como array para soportar múltiples periodos
    if (req.query.cveper) {
      activeFilters.cveper = Array.isArray(req.query.cveper) ? req.query.cveper : [req.query.cveper];
      console.log('📅 DEBUG cveper procesado:', activeFilters.cveper);
    }
...
app.get('/api/payroll/demographic/unique-count', async (req, res) => {
  try {
    ...
    console.log('🔢 /api/payroll/demographic/unique-count: Parámetros recibidos:', {
...
app.get('/api/documents/search-by-name', async (req, res) => {
  try {
    ...
    if (result.success) {
      console.log(`📁 Encontrados ${result.files.length} documentos para ${employeeName}`);
...
app.get('/api/documents/search-subfolders', async (req, res) => {
  try {
    ...
    if (result.success) {
      console.log(`📁 Encontrados ${result.files.length} documentos en subcarpetas`);
...
app.get('/api/debug/employee-fields', async (req, res) => {
  try {
    ...
    console.log('🔍 DEBUG: Consultando campos para CURP:', curp, 'Período:', cveper);
  ...
  try {
    ...
    try {
      ...
      const percepcionFields = [
        ...
        'PRIMA DOMINICAL', 'PRIMA DE ANTIGÜEDAD', 'DIA FESTIVO TRABAJADO',
        'SEPTIMO DIA', 'VACACIONES PENDIENTES', 'DESTAJO', 'DESTAJO INFORMADO',
        'PTU', 'REGALÍAS', 'SUBSIDIO AL EMPLEO', 'SUBSIDIO POR INCAPACIDAD',
      ...
      const deduccionFields = [
        ...
        'PRESTAMOS PERSONALES', 'PRESTAMO FPL', 'PENSIÓN ALIMENTICIA',
      ...
      console.log('🔍 DEBUG - Campos disponibles en employeeData:', Object.keys(employeeData));
      ...
      console.log('✅ /api/payroll/employee-data: Datos estructurados exitosamente:', {
...
app.get('/api/percepciones', async (req, res) => {
  try {
    ...
    console.log('💰 /api/percepciones: Parámetros recibidos:', { curp, cveper, pageSize, page });
...
// ⚠️ IMPORTANT: This route must be defined AFTER all specific /api/payroll/* routes
app.get('/api/payroll', async (req, res) => {
  // ✅ CHECK: If this is actually a request for a specific payroll sub-route, return 404
  // This should never happen if routes are registered correctly, but adding as safety check
  if (req.originalUrl.includes('/rfc-from-curp') || 
      req.originalUrl.includes('/name-from-curp') || 
      req.originalUrl.includes('/periodos-from-curp') ||
      req.originalUrl.includes('/employee-data')) {
    console.error('⚠️ WARNING: /api/payroll route matched but URL suggests specific route:', req.originalUrl);
    return res.status(404).json({
      success: false,
      error: 'Route not found - specific route should have matched first'
    });
  }
  ...
  try {
    // ✅ FIXED: Extract query parameters - support both new (sortField/sortOrder/limit) and old (orderBy/orderDirection/pageSize) names
    const { 
      pageSize, limit, // Support both pageSize and limit
      page, 
      search, puesto, compania, sucursal, status, puestoCategorizado, cveper, 
      sortField, orderBy, // Support both sortField and orderBy (sortField takes priority)
      sortOrder, orderDirection, // Support both sortOrder and orderDirection (sortOrder takes priority)
      fullData 
    } = req.query;
    
    // ✅ Normalize parameters: use new names if provided, fallback to old names
    const normalizedPageSize = limit || pageSize;
    const normalizedSortField = sortField || orderBy;
    const normalizedSortOrder = sortOrder || orderDirection;
    
    // 🔍 CRITICAL: Log what sorting parameters are received from frontend
    console.log('\n🔵🔵🔵 [FRONTEND REQUEST DEBUG] 🔵🔵🔵');
    console.log('🔵 Full req.query:', JSON.stringify(req.query, null, 2));
    console.log('🔵 sortField from frontend:', JSON.stringify(sortField));
    console.log('🔵 sortOrder from frontend:', JSON.stringify(sortOrder));
    console.log('🔵 orderBy (legacy) from frontend:', JSON.stringify(orderBy));
    console.log('🔵 orderDirection (legacy) from frontend:', JSON.stringify(orderDirection));
    console.log('🔵 Normalized sortField:', JSON.stringify(normalizedSortField));
    console.log('🔵 Normalized sortOrder:', JSON.stringify(normalizedSortOrder));
    console.log('🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵\n');
    ...
    console.log('📥 [API REQUEST] Raw query parameters received:', {
      ...
      cveper: cveper || 'NONE',
      sortField: normalizedSortField || 'NONE',
      sortOrder: normalizedSortOrder || 'NONE',
      orderBy: orderBy || 'NONE (legacy)',
      orderDirection: orderDirection || 'NONE (legacy)',
    ...
    // 🔍 LOG: Show exact sorting parameters received
    console.log('\n🔵 [SERVER - SORTING DEBUG]');
    console.log('🔵 Raw sortField from req.query:', JSON.stringify(sortField));
    console.log('🔵 Raw sortOrder from req.query:', JSON.stringify(sortOrder));
    console.log('🔵 Raw orderBy (legacy) from req.query:', JSON.stringify(orderBy));
    console.log('🔵 Raw orderDirection (legacy) from req.query:', JSON.stringify(orderDirection));
    console.log('🔵 Normalized sortField:', JSON.stringify(normalizedSortField));
    console.log('🔵 Normalized sortOrder:', JSON.stringify(normalizedSortOrder));
    
    // Build options object with normalized values
    const includeAllFields = req.query.includeAllFields === 'true' || req.query.includeAllFields === true;
    const serviceOptions = {
      pageSize: parseInt(normalizedPageSize) || 100,
      page: parseInt(page) || 1,
      orderBy: normalizedSortField || null, // Use normalized sortField
      orderDirection: normalizedSortOrder || null, // Use normalized sortOrder
      fullData: fullData === 'true' || fullData === true,
      includeAllFields: includeAllFields // ✅ Pass includeAllFields to service
    };
    
    console.log('🔵 serviceOptions.orderBy (normalized from sortField):', JSON.stringify(serviceOptions.orderBy));
    console.log('🔵 serviceOptions.orderDirection (normalized from sortOrder):', JSON.stringify(serviceOptions.orderDirection));
    ...
    if (cleanedSearch || normalizedSortField || puesto || sucursal || status || puestoCategorizado || cveper) {
      console.log('🔍 [FILTER/SORT] Active filters and sorting:', {
        ...
        sortOrder: normalizedSortOrder || null,
    ...
    if (cleanedSearch) {
      console.log('✅ [SERVICE CALL] Passing search to service:', {
        original: search,
        cleaned: cleanedSearch,
      ...
      console.warn('⚠️ [SERVICE CALL] Search parameter received but cleanedSearch is null:', {
        original: search,
        cleaned: cleanedSearch,
    ...
    console.log('🔵 ABOUT TO CALL SERVICE - serviceOptions.search =', serviceOptions.search);
    console.log('🔵 ABOUT TO CALL SERVICE - Full serviceOptions =', JSON.stringify(serviceOptions, null, 2));
    
    const result = await payrollFilterService.getPayrollDataWithFiltersAndSorting(serviceOptions);
    
    console.log('🟢 SERVICE RETURNED - result.success =', result.success);
    console.log('🟢 SERVICE RETURNED - result.data.length =', result.data?.length || 0);
    console.log('🟢 SERVICE RETURNED - result.total =', result.total || 0);
    
    console.log('✅ [SERVICE RESULT] Service returned data:', {
      success: result.success,
      totalRecords: result.total || 0,
      dataLength: result.data?.length || 0,
      pagination: result.pagination || 'NONE',
      firstRecordSample: result.data && result.data.length > 0 ? {
        nombre: result.data[0].nombre,
        curp: result.data[0].curp,
        rfc: result.data[0].rfc
      } : 'NO DATA'
    });
...
// ✅ FIXED: Obtener datos de fondos por RFC (endpoint simple para frontend) - MUST BE BEFORE /api/fondos/tables
// Route registered at: Line 2509
app.get('/api/fondos', async (req, res) => {
  // ✅ VERIFICATION: This log confirms the route is being hit
  console.log('\n🔵🔵🔵 [FONDOS ENDPOINT CALLED] 🔵🔵🔵');
  console.log('🔵 Full URL:', req.protocol + '://' + req.get('host') + req.originalUrl);
  console.log('🔵 Query params:', req.query);
  console.log('🔵 RFC received:', req.query.rfc);
...
app.get('/api/fondos/tables/:tableName/structure', verifyToken, requireFundsPermission, async (req, res) => {
  try {
    ...
    console.error('❌ Error obteniendo estructura de tabla de fondos:', error);
    res.status(500).json({
      success: false,
      error: error.message
    });
...
// ============================================================================
// RUTAS DE AUTENTICACIÓN PÚBLICA - Registro y validación
// ============================================================================
...
// Actualizar status del usuario después de registro exitoso en Cognito
app.post('/api/auth/confirm-registration', async (req, res) => {
  try {
    const { email, firstName, lastName, phoneNumber } = req.body;
    ...
    try {
      ...
      console.log('✅ Registro confirmado para:', email);
...
app.post('/api/upload-data', upload.single('file'), async (req, res) => {
  try {
    ...
    if (result.success) {
      console.log(`✅ Upload exitoso: ${result.recordsInserted} registros insertados`);
      res.json({
        success: true,
...
app.use((req, res) => {
  // ✅ LOGGING: Track what routes are hitting 404
  console.log('\n❌❌❌ [404 HANDLER TRIGGERED] ❌❌❌');
  console.log('❌ Method:', req.method);
  console.log('❌ Path:', req.path);
  console.log('❌ Original URL:', req.originalUrl);
  console.log('❌ Query:', req.query);
  console.log('❌ This route was NOT matched by any registered route!');
  console.log('❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌ function call exceeded the maximum number of tokens. Please shorten the output.
