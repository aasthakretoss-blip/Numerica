...
async function testPayrollWithPeriodFilter() {
  try {
    ...
    const periodsResponse = await fetch('http://localhost:3001/api/payroll/periodos');
    ...
    if (periodsResult.success && periodsResult.data.length > 0) {
      ...
      const response = await fetch(`http://localhost:3001/api/payroll?cveper=${encodeURIComponent(testPeriod)}&pageSize=5`);
      const result = await response.json();
