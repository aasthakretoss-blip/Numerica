...
async function testPeriodsEndpoint() {
  try {
    ...
    const response = await fetch(`${process.env.REACT_APP_API_URL}/api/payroll/periodos`);
...
async function testPayrollWithPeriodFilter() {
  try {
    ...
    const periodsResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/payroll/periodos`);
    ...
    if (periodsResult.success && periodsResult.data.length > 0) {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/payroll?cveper=${encodeURIComponent(testPeriod)}&pageSize=5`);
      const result = await response.json();
