...
async function diagnosticoPeriodo() {
  ...
  try {
    ...
    if (
      ...
      console.log(
        `✅ Períodos cargados: ${periodsResult.data.length} períodos encontrados`
      );

      // Ordenar períodos como lo hace el código
      const sortedPeriods = periodsResult.data.sort(
        ...
        try {
          ...
          if (response.ok && result.success) {
            if (testCase.name === "Filter Options") {
              console.log(
                `   ✅ Success: ${
                  result.data?.sucursales?.length || 0
                } sucursales, ${result.data?.puestos?.length || 0} puestos`
              );
            } else if (testCase.name === "Demographic Data") {
              console.log(
                `   ✅ Success: ${result.data?.length || 0} registros de ${
                  result.total || 0
                } total`
              );
            } else if (testCase.name === "Unique Count") {
              console.log(
                `   ✅ Success: ${result.uniqueCurpCount || 0} empleados únicos`
              );
            }
          } else {
      ...
      for (const format of formats) {
        ...
        try {
          const response = await fetch(
            buildApiUrl(
              `/api/payroll/demographic/unique-count?cveper=${format}&status=A`
            )
          );
          const result = await response.json();
          ...
          if (response.ok && result.success) {
            console.log(
              `   ✅ ${result.uniqueCurpCount || 0} empleados únicos`
            );
          } else {
            console.log(`   ❌ Error: ${result.error || "Error desconocido"}`);
          }
      ...
      try {
        const response = await fetch(
          buildApiUrl("/api/payroll/demographic/unique-count?status=A")
        );
        const result = await response.json();
        ...
        if (response.ok && result.success) {
          ...
          const withFilter = await fetch(
            buildApiUrl(
              `/api/payroll/demographic/unique-count?cveper=${monthFilter}&status=A`
            )
          );
          ...
          if (withFilter.ok && withFilterResult.success) {
            ...
            console.log(
              `      - Diferencia: ${
                withoutFilterCount - withFilterCount
              } empleados`
            );

            if (withFilterCount === 0) {
              console.log(
                "   ⚠️  PROBLEMA: El filtro de período está filtrando todos los datos"
              );
            } else if (withFilterCount === withoutFilterCount) {
              console.log(
                "   ⚠️  PROBLEMA: El filtro de período no está teniendo efecto"
              );
            } else {
              console.log(
                "   ✅ El filtro de período está funcionando correctamente"
              );
            }
          }
        }
  ...
  console.log(
    "1. Si el filtro devuelve 0 empleados, verificar el formato de fecha en la API"
  );
  console.log(
    "2. Si el filtro no tiene efecto, verificar el parámetro cveper en el backend"
  );
  console.log(
    "3. Verificar que el campo cveper en la BD tenga el formato esperado"
  );
...
if (typeof window !== "undefined") {
  ...
  console.log(
    "🚀 Diagnóstico de período cargado. Ejecutar: diagnosticoPeriodo()"
  );
}
