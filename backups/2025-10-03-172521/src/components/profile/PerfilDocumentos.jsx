...
const PerfilDocumentos = ({ rfc }) => {
  ...
  const searchDocuments = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/documents/search-by-name?employeeName=${encodeURIComponent(nombreData.nombreCompleto)}`
      );
  ...
  const downloadDocument = async (documentFile) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/documents/download/${documentFile.id}`
      );
  ...
  useEffect(() => {
    const fetchNombreCompleto = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/payroll/name-from-curp?curp=${encodeURIComponent(curpFromURL)}`);
