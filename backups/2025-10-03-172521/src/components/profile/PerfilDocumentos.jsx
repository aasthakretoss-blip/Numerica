...
const PerfilDocumentos = ({ rfc }) => {
  ...
  const searchDocuments = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://numericaapi.kretosstechnology.com:3001/api/documents/search-by-name?employeeName=${encodeURIComponent(nombreData.nombreCompleto)}`
      );
  ...
  const viewDocument = async (document) => {
    try {
      ...
      const response = await fetch(
        `http://numericaapi.kretosstechnology.com:3001/api/documents/download/${document.id}`
      );
  ...
  const downloadDocument = async (documentFile) => {
    try {
      ...
      const response = await fetch(
        `http://numericaapi.kretosstechnology.com:3001/api/documents/download/${documentFile.id}`
      );
  ...
  useEffect(() => {
    const fetchNombreCompleto = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://numericaapi.kretosstechnology.com:3001/api/payroll/name-from-curp?curp=${encodeURIComponent(curpFromURL)}`);
