  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        const fechasResponse = await fetch(`http://localhost:3001/api/payroll/fecpla-from-rfc?rfc=${encodeURIComponent(rfc)}`);
        ...
        const dataPromises = fechasFPL.map(async (fechaFPL, index) => {
          ...
          try {
            const dataUrl = `http://localhost:3001/api/fpl/data-from-rfc?rfc=${encodeURIComponent(rfc)}&originalFecpla=${encodeURIComponent(metadata.originalFecpla)}&originalAntiguedad=${encodeURIComponent(metadata.originalAntiguedad)}`;
