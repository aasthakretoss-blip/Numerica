...
import authenticatedFetch from "../../../../../../src/services/authenticatedFetch";
...
const PercepcionesHistoricoChart = ({ curp, onPeriodClick }) => {
    ...
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        if (!result.success || !result.data || result.data.length === 0) {
          throw new Error(
            "No se encontraron datos históricos para este empleado"
          );
        }

        console.log(
          "📊 [Chart] Datos obtenidos:",
          result.data.length,
          "registros"
        );
  ...
  const processChartData = (data) => {
    ...
    data.forEach((record) => {
      ...
      const totalPercepciones = parseFloat(
        record[" TOTAL DE PERCEPCIONES "] || 0
      );
      const sueldoCliente = parseFloat(
        record["SUELDO CLIENTE"] ||
          record[" SUELDO CLIENTE "] ||
          record["SUELDO_CLIENTE"] ||
          0
      );
      ...
      console.log("🔍 [Chart] Registro:", {
        ...
        totalComisiones,
        camposComisiones: Object.keys(record)
          .filter((key) => key.toLowerCase().includes("comision"))
          .slice(0, 10),
      });
    ...
    const comisionesTotalesData = sortedData.map(
      (item) => item.totalComisiones
    );
  ...
  const generateChartOptions = (data) => {
    ...
    const ySteps = Math.min(
      10,
      Math.max(5, Math.ceil(range / (maxValue * 0.1)))
    );
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: function (context) {
              ...
              if (
                datasetLabel === "COMISIONES TOTALES" &&
                data.metadata.comisionesDetalle
              ) {
                const detalle = data.metadata.comisionesDetalle[dataIndex];
                if (detalle) {
                  return [
                    `${datasetLabel}: $${value.toLocaleString("es-MX", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}`,
                    `  • Comisiones Cliente: $${detalle.comisionesCliente.toLocaleString(
                      "es-MX",
                      { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                    )}`,
                    `  • Comisiones Facturadas: $${detalle.comisionesFacturadas.toLocaleString(
                      "es-MX",
                      { minimumFractionDigits: 2, maximumFractionDigits: 2 }
                    )}`,
                  ];
  ...
  if (loading) {
    return (
      <ChartContainer>
        ...
        <LoadingContainer>Cargando datos históricos...</LoadingContainer>
      </ChartContainer>
    );
  }
  ...
  if (!chartData) {
    return (
      <ChartContainer>
        <ChartTitle>Histórico de Percepciones</ChartTitle>
        <LoadingContainer>No hay datos disponibles</LoadingContainer>
      </ChartContainer>
    );
  }
