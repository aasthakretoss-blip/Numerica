...
const StatusBadge = styled.span`
  ...
  background: ${
    props => {
      switch (props.$status) {
        case 'Activo': return 'rgba(46, 204, 113, 0.2)';
        case 'Baja': return 'rgba(231, 76, 60, 0.2)';
        case 'Finiquitado': return 'rgba(241, 196, 15, 0.2)';
  ...
  color: ${
    props => {
      switch (props.$status) {
        ...
        case 'Baja': return '#e74c3c';
        case 'Finiquitado': return '#f39c12';
  ...
  border: 1px solid ${
    props => {
      switch (props.$status) {
        case 'Activo': return 'rgba(46, 204, 113, 0.3)';
        case 'Baja': return 'rgba(231, 76, 60, 0.3)';
        case 'Finiquitado': return 'rgba(241, 196, 15, 0.3)';
...
const PaginationButton = styled.button`
  background: ${props => props.$active ? 'rgba(30, 58, 138, 0.3)' : 'rgba(255, 255, 255, 0.1)'};
  border: 1px solid ${props => props.$active ? '#1e3a8a' : 'rgba(255, 255, 255, 0.2)'};
  border-radius: 6px;
  padding: 0.5rem 0.75rem;
...
export default function TablaDemografico({ 
  ...
  const loadData = async () => {
    try {
      ...
      console.log('📊 TablaDemografico: Aplicando filtros demográficos:', {
        filterParams,
        additionalParams,
        finalUrl: `http://numericaapi.kretosstechnology.com:3001/api/payroll/demographic?${params.toString()}`
      });
      ...
      const [dataResponse, _] = await Promise.all([
        fetch(`http://numericaapi.kretosstechnology.com:3001/api/payroll/demographic?${params}`),
      ...
      if (dataResponse.ok) {
        ...
        console.error('❌ Error HTTP:', dataResponse.status, dataResponse.statusText);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (periodFilter !== null) {
      console.log('🔄 TablaDemografico: Recargando datos por cambio de filtros:', {
  ...
  const toggleSort = (key) => {
    console.log('🔄 TablaDemografico.toggleSort llamado:', { key, sortBy, sortDir, localSortBy, localSortDir });
  ...
  const handleViewEmployee = (employee) => {
    ...
    const identifier = employee.curp?.trim() || employee.rfc?.trim();
    let navigationPath;
    
    if (identifier) {
      navigationPath = `/perfil/${identifier}`;
    } else {
      // Fallback: usar el nombre del empleado limpio
      const safeName = employee.nombre
        ?.replace(/\s+/g, '-')
        ?.toLowerCase()
  ...
  const transformedEmployees = useMemo(() => {
    const transformedData = employees.map(emp => ({
      ...
      // COALESCE(" TOTAL DE PERCEPCIONES ", 0) as "totalPercepciones", "Status" as status
      ...
      total: emp.totalPercepciones || 0,                   // CORREGIDO: usar totalPercepciones del endpoint
    ...
    if (['salario', 'comisiones', 'total'].includes(localSortBy)) {
      console.log('🔢 TablaDemografico: Aplicando local sorting para campo numérico:', localSortBy);
    ...
    <TableContainer>
      <TableHeader $collapsed={collapsed}>
        <TableTitle>
          ...
          {!collapsed && (
            <div style={{ color: 'rgba(44, 62, 80, 0.7)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <span>
                {pagination.total} registros {periodFilter && `(${periodFilter})`}
              </span>
              <span style={{
                background: 'rgba(30, 58, 138, 0.2)',
                padding: '0.25rem 0.75rem',
                  ...
                  {transformedEmployees.length === 0 && !loading && (
                    <tr>
                      <TableCell colSpan={columns.length}>
                        <NoResultsContainer>
                          <h3 style={{ marginBottom: '0.5rem' }}>No se encontraron datos</h3>
              ...
              <PaginationControls>
                ...
                {generatePageNumbers().map((pageNum, index) => (
                  typeof pageNum === 'number' ? (
                    ...
                    <span key={index} style={{ color: 'rgba(44, 62, 80, 0.5)', padding: '0 0.5rem' }}>
                      {pageNum}
                    </span>
                  )
                ))}
