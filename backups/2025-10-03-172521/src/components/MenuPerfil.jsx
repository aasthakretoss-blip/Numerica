...
const MenuButton = styled.button`
  ...
  &:hover {
    ...
    box-shadow: ${props => props.theme?.effects?.shadows?.medium || '0 8px 25px rgba(0, 0, 0, 0.15)'};
  ...
  &:active {
    ...
    box-shadow: ${props => props.theme?.effects?.shadows?.small || '0 4px 15px rgba(0, 0, 0, 0.1)'};
  }
...
const MenuPerfil = ({ rfc }) => {
  const { theme } = useTheme();
  const navigate = useNavigate();
  const location = useLocation();
  
  // Determinar qué tab está activa basada en la URL
  const activeTab = location.pathname.includes('/fpl') ? 'fpl' 
    : location.pathname.includes('/historico') ? 'historico'
    : location.pathname.includes('/documentos') ? 'documentos'
    : 'empleado';

  const handleTabChange = (tabKey) => {
    if (!rfc) return;
    
    const basePath = `/perfil/${rfc}`;
