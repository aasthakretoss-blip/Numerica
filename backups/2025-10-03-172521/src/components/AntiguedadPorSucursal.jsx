...
const SENIORITY_BANDS = [
  { min: 0, max: 1, label: "< 1 año", color: "linear-gradient(90deg, #2E8B57, #3CB371)", name: "Nuevo" },
  { min: 1, max: 3, label: "1-3 años", color: "linear-gradient(90deg, #1e3a8a, #3b82f6)", name: "Junior" },
  { min: 3, max: 5, label: "3-5 años", color: "linear-gradient(90deg, #FFD700, #FFA500)", name: "Intermedio" },
  { min: 5, max: 10, label: "5-10 años", color: "linear-gradient(90deg, #FF6347, #DC143C)", name: "Senior" },
  { min: 10, max: Infinity, label: "10+ años", color: "linear-gradient(90deg, #8B008B, #4B0082)", name: "Veterano" }
];
...
export default function AntiguedadPorSucursal({ 
  ...
  const getYearsOfSeniority = (emp) => {
    const seniorityFields = ['Fecha antigüedad', 'fecha_antiguedad', 'fechaAntiguedad', 'start_date', 'hire_date'];
  ...
  const getSeniorityBand = (years) => {
    ...
    return SENIORITY_BANDS.find(band => years >= band.min && years < band.max) || SENIORITY_BANDS[0];
  };
  ...
  const loadAllEmployees = async () => {
    try {
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            if (currentPage === 1) {
              ...
              console.log(`📊 DistribuciónPorSucursal: Total esperado: ${expectedTotal}, Páginas: ${totalPages}`);
            ...
            if (result.data && result.data.length > 0) {
              ...
              console.log(`✅ DistribuciónPorSucursal: Página ${currentPage} cargada - ${result.data.length} registros (Total acumulado: ${allEmployeesData.length})`);
            } else {
              console.log(`⚠️ DistribuciónPorSucursal: Página ${currentPage} vacía`);
              break;
            }
            
            currentPage++;
          } else {
            console.error(`❌ DistribuciónPorSucursal: Error en página ${currentPage}:`, result.error);
            break;
          }
        } else {
          console.error(`❌ DistribuciónPorSucursal: Error HTTP en página ${currentPage}:`, response.status, response.statusText);
          break;
        }
      ...
      console.error('❌ DistribuciónPorSucursal: Error loading demographic data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const seniorityData = useMemo(() => {
    if (!allEmployees.length) {
      console.log('📊 DistribuciónPorSucursal - Sin empleados cargados, retornando estructura vacía');
      return { branchData: {}, stats: { totalEmployees: 0, totalBranches: 0 } };
    }

    console.log('📊 DistribuciónPorSucursal - INICIANDO procesamiento de datos:');
    ...
    console.log(`📊 DistribuciónPorSucursal - Total registros originales: ${allEmployees.length}`);
    console.log(`📊 DistribuciónPorSucursal - CURPs únicos identificados: ${uniqueEmployees.length}`);
      ...
      if (seniorityBand && yearsOfSeniority !== null) {
        ...
        if (index < 10) {
          console.warn(`⚠️ Empleado ${index + 1} con antigüedad inválida:`, {
            ...
            yearsOfSeniority,
            seniorityBand,
            camposAntiguedad: ['Fecha antigüedad', 'fecha_antiguedad', 'fechaAntiguedad'].map(f => `${f}: ${emp[f]}`)
          });
        }
      }
    });
    ...
    console.log('- Sucursales ordenadas:', sortedBranches.slice(0, 10).join(', '), sortedBranches.length > 10 ? '...' : '');
  ...
  const maxEmployeeCount = useMemo(() => {
    ...
    console.log(`📊 DistribuciónPorSucursal - Máximo empleados por sucursal: ${max}`);
    return max;
      ...
      <ChartContent $collapsed={collapsed}>
        {loading ? (
          <LoadingContainer>
            ...
            <p style={{ marginTop: '1rem' }}>Analizando distribución por sucursal...</p>
          </LoadingContainer>
        ) : (
          <ChartWrapper>
                          ...
                          {SENIORITY_BANDS.map(band => {
                            ...
                            return (
                              <BarSegment
                                ...
                                style={{ width: `${width}%` }}
                              >
                                {count > 3 && count} {/* Solo mostrar números si hay suficiente espacio */}
                                <Tooltip>
                                  {branchName}<br/>
