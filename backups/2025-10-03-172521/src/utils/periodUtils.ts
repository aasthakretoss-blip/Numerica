...
export const normalizeCveperDate = (cveper: string): string => {
  console.log('🔍 normalizeCveperDate - INPUT:', {
    cveper,
    tipo: typeof cveper,
    longitud: cveper ? cveper.length : 0
  });
  
  if (!cveper || typeof cveper !== 'string') {
    console.log('❌ normalizeCveperDate - RETORNANDO VACÍO por valor inválido:', cveper);
    return '';
  }
  ...
  try {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(cveper)) {
      console.log('✅ normalizeCveperDate - Ya en formato YYYY-MM-DD:', cveper);
      return cveper;
    }

    // Si es un timestamp ISO, convertirlo
    if (cveper.includes('T') || cveper.includes('Z')) {
      console.log('🕒 normalizeCveperDate - Detectado timestamp ISO, convirtiendo...');
      const date = new Date(cveper);
      if (!isNaN(date.getTime())) {
        const resultado = date.toISOString().split('T')[0];
        console.log('✅ normalizeCveperDate - Timestamp convertido:', {
          original: cveper,
          convertido: resultado
        });
        return resultado;
      } else {
        console.log('❌ normalizeCveperDate - Timestamp inválido, usando original:', cveper);
      }
    }

    // Si es otro formato, intentar parsearlo
    console.log('🔄 normalizeCveperDate - Intentando parsear como fecha genérica...');
    const date = new Date(cveper);
    if (!isNaN(date.getTime())) {
      const resultado = date.toISOString().split('T')[0];
      console.log('✅ normalizeCveperDate - Fecha parseada exitosamente:', {
        original: cveper,
        convertido: resultado
      });
      return resultado;
    ...
    console.log('⚠️ normalizeCveperDate - No se pudo parsear, retornando original:', cveper);
...
export const formatCveperForTable = (cveper: any): string => {
  console.log('🔍 formatCveperForTable - INPUT:', {
    cveper,
    tipo: typeof cveper,
    esNull: cveper === null,
    esUndefined: cveper === undefined,
    esStringVacio: cveper === '',
    longitud: cveper ? cveper.toString().length : 0
  });
  
  // Si es null, undefined o vacío
  if (!cveper) {
    console.log('❌ formatCveperForTable - RETORNANDO vacío porque cveper es falsy:', cveper);
    return '';
  }
  
  try {
    // Convertir a string si no lo es
    const cveperStr = cveper.toString();
    
    // Si ya está en formato YYYY-MM-DD, retornarlo
    if (/^\d{4}-\d{2}-\d{2}$/.test(cveperStr)) {
      console.log('✅ formatCveperForTable - Ya en formato correcto:', cveperStr);
      return cveperStr;
    }
    ...
    if (!isNaN(date.getTime())) {
      const resultado = date.toISOString().split('T')[0];
      console.log('✅ formatCveperForTable - Timestamp convertido exitosamente:', {
        original: cveper,
        convertido: resultado
      });
      return resultado;
    ...
    if (dateMatch) {
      const resultado = `${dateMatch[1]}-${dateMatch[2]}-${dateMatch[3]}`;
      console.log('✅ formatCveperForTable - Fecha extraída con regex:', resultado);
      return resultado;
    }
    
    console.log('⚠️ formatCveperForTable - No se pudo convertir, retornando string original:', cveperStr);
    return cveperStr;
    
  } catch (error) {
    console.error('❌ formatCveperForTable - Error procesando cveper:', cveper, error);
    return cveper ? cveper.toString() : '';
  }
};
