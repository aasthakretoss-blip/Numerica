...
async function testUploadEndpoints() {
  ...
  try {
    ...
    const infoResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/info`);
  ...
  try {
    ...
    const validateResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/validate-file`, {
      method: 'POST',
      body: new FormData()
    });
  ...
  try {
    ...
    const uploadResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/upload-data`, {
      method: 'POST',
      body: new FormData()
    });
