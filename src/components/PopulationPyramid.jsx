...
export default function PopulationPyramid({
  ...
  const loadLatestPeriod = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          ...
          if (isNaN(periodDate.getTime())) {
            console.warn(
              "⚠️ PopulationPyramid - Fecha del período inválida:",
              latest.value
            );
            // Usar período por defecto
            const defaultPeriod = "2024-10";
            console.log(
              "📅 PopulationPyramid - Usando período por defecto:",
              defaultPeriod
            );
          ...
          console.log(
            "📅 PopulationPyramid - Último período encontrado:",
            latest.value,
            "Filtro aplicado:",
            monthFilter
          );
          ...
          console.warn(
            "⚠️ PopulationPyramid - No se encontraron períodos válidos"
          );
          // Usar período por defecto
          const defaultPeriod = "2024-10";
          console.log(
        ...
        console.warn(
          "⚠️ PopulationPyramid - Error en respuesta del endpoint:",
          response.status
        );
        // Usar período por defecto
        const defaultPeriod = "2024-10";
        console.log(
          "📅 PopulationPyramid - Usando período por defecto tras error HTTP:",
          defaultPeriod
        );
        setLatestPeriod({ value: "2024-10-01", label: "Octubre 2024" });
        setPeriodFilter(defaultPeriod);
      ...
      console.error(
        "❌ PopulationPyramid - Error loading latest period:",
        error
      );
      // Usar período por defecto en caso de error
      const defaultPeriod = "2024-10";
      console.log(
        "📅 PopulationPyramid - Usando período por defecto tras error:",
        defaultPeriod
      );
      setLatestPeriod({ value: "2024-10-01", label: "Octubre 2024" });
      setPeriodFilter(defaultPeriod);
  ...
  const loadUniqueCurpCount = async () => {
    try {
      ...
      console.log(
        "🔍 PopulationPyramid: Contando CURPs únicos con filtros:",
        filterParams
      );
      ...
      if (response.ok) {
        ...
        if (result.success) {
          console.log(
            "🔍 DEBUG: Actualizando totalUniqueEmployees de",
  ...
  const loadAllEmployees = async () => {
    try {
      ...
      do {
        ...
        console.log(
          `📄 PopulationPyramid: Cargando página ${currentPage}/${totalPages}...`
        );

        // CORRECCIÓN: Usar endpoint existente /api/payroll en lugar del demographic inexistente
        console.log(
          "🔄 PopulationPyramid - REDIRIGIENDO a endpoint existente /api/payroll"
        );
        const response = await authenticatedFetch(
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            if (currentPage === 1) {
              ...
              console.log(
                `📊 PopulationPyramid: Total esperado: ${expectedTotal}, Páginas: ${totalPages}`
              );
            ...
            if (result.data && result.data.length > 0) {
              ...
              console.log(
                `✅ PopulationPyramid: Página ${currentPage} cargada - ${result.data.length} registros (Total acumulado: ${allEmployeesData.length})`
              );
            } else {
            ...
            console.error(
              `❌ PopulationPyramid: Error en página ${currentPage}:`,
              result.error
            );
            break;
          }
          ...
          console.error(
            `❌ PopulationPyramid: Error HTTP en página ${currentPage}:`,
            response.status,
            response.statusText
          );
          break;
      ...
      console.error(
        "❌ PopulationPyramid: Error loading demographic data:",
        error
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    if (periodFilter !== null) {
      console.log(
        "🔄 PopulationPyramid: Cargando datos completos - Period Filter:",
        periodFilter
      );
      loadAllEmployees();
    }
  ...
  const populationData = useMemo(() => {
    if (!allEmployees.length) {
      console.log(
        "📊 PopulationPyramid - Sin empleados cargados, retornando estructura vacía"
      );
      return {
        ageGroups: {},
        stats: {
    ...
    console.log(
      `📊 PopulationPyramid - Total registros originales: ${allEmployees.length}`
    );
    console.log(
      `📊 PopulationPyramid - CURPs únicos identificados: ${uniqueEmployees.length}`
    );
    ...
    console.log(
      "📊 PopulationPyramid - Empleados procesados:",
      processedEmployees.length
    );
    console.log(
      "📊 PopulationPyramid - Conteos por género procesados - M:",
      processedMaleCount,
      "F:",
      processedFemaleCount
    );
    ...
    const stats = {
      ...
      malePercentage:
        processedMaleCount + processedFemaleCount > 0
          ? (processedMaleCount / (processedMaleCount + processedFemaleCount)) *
            100
          : 0,
      femalePercentage:
        processedMaleCount + processedFemaleCount > 0
          ? (processedFemaleCount /
              (processedMaleCount + processedFemaleCount)) *
            100
          : 0,
    };
  ...
  const { maxMales, maxFemales, maxCount } = useMemo(() => {
    ...
    console.log(
      `📊 Máximos por género - Hombres: ${maxMales}, Mujeres: ${maxFemales}, Absoluto: ${absoluteMax}`
    );
  ...
  const uniformAxisLimit = useMemo(() => {
    ...
    console.log(
      `📊 ESCALA UNIFORME - Máximo real: ${maxCount}, Límite aplicado: ${limit}`
    );
    console.log(
      `📊 Ambos lados usarán la misma escala para proporcionalidad correcta`
    );
  ...
  const { dynamicMinAge, dynamicMaxAge } = useMemo(() => {
    ...
    if (!ageGroups || Object.keys(ageGroups).length === 0) {
      console.log(
        `📊 PopulationPyramid - Sin datos, usando rango por defecto: ${minAge}-${maxAge}`
      );
    ...
    if (agesWithData.length === 0) {
      console.log(
        `📊 PopulationPyramid - No hay edades con datos, usando rango por defecto: ${minAge}-${maxAge}`
      );
      return { dynamicMinAge: minAge, dynamicMaxAge: maxAge };
    }
    ...
    console.log(
      `📊 PopulationPyramid - Rango detectado: ${detectedMinAge}-${detectedMaxAge}, usando: ${finalMinAge}-${finalMaxAge}`
    );
    console.log(
      `📊 PopulationPyramid - Edades con datos: ${agesWithData.join(", ")}`
    );
  ...
  const ageRange = useMemo(() => {
    ...
    console.log(
      `📊 PopulationPyramid - Rango dinámico: ${dynamicMinAge}-${dynamicMaxAge} años`
    );
            ...
            <LeftSidebar>
              <StatsContainer>
                <StatItem>
                  <StatValue>
                    {totalUniqueEmployees ||
                      populationData.stats?.totalEmployees ||
                      0}
                  </StatValue>
                ...
                <StatItem>
                  <StatValue>
                    {populationData.stats?.totalFemale || 0}
                  </StatValue>
                  <StatLabel>
                    Mujeres (
                    {populationData.stats?.totalEmployees > 0
                      ? populationData.stats.femalePercentage.toFixed(1)
                      : "0.0"}
                    %)
                  </StatLabel>
                </StatItem>
                <StatItem>
                  ...
                  <StatLabel>
                    Hombres (
                    {populationData.stats?.totalEmployees > 0
                      ? populationData.stats.malePercentage.toFixed(1)
                      : "0.0"}
                    %)
                  </StatLabel>
                </StatItem>
                <StatItem>
                  <StatValue>
                    {(populationData.stats?.averageAge || 0).toFixed(1)}
                  </StatValue>
            ...
            <ChartContainer>
              <PyramidChart>
                {ageRange.map((age) => {
                  ...
                  const data = populationData.ageGroups[age] || {
                    male: 0,
                    female: 0,
                  };
                  ...
                  const femaleWidth =
                    uniformAxisLimit > 0
                      ? (data.female / uniformAxisLimit) * 100
                      : 0;
                  const maleWidth =
                    uniformAxisLimit > 0
                      ? (data.male / uniformAxisLimit) * 100
                      : 0;
                  ...
                  if (data.female > 0 || data.male > 0) {
                    console.log(
                      `🎨 Edad ${age}: F=${
                        data.female
                      }/${uniformAxisLimit}=${femaleWidth.toFixed(1)}%, M=${
                        data.male
                      }/${uniformAxisLimit}=${maleWidth.toFixed(1)}%`
                    );
                  }
                    ...
                    <AgeRow key={age}>
                      <LeftContainer>
                        {data.female > 0 && (
                          ...
                          >
                            <TooltipBar>
                              {age} años: {data.female} mujeres (clic para
                              detalles)
                            </TooltipBar>
                          </LeftBar>
                        )}
                      ...
                      <RightContainer>
                        {data.male > 0 && (
                          ...
                          >
                            <TooltipBar>
                              {age} años: {data.male} hombres (clic para
                              detalles)
                            </TooltipBar>
                          </RightBar>
                        )}
