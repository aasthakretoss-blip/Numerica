...
const PayrollDataViewer = ({ curp, selectedPeriod }) => {
  ...
  const fetchPayrollData = useCallback(async (curpValue, periodValue) => {
    ...
    try {
      ...
      if (periodValue) {
        let cveperValue = Array.isArray(periodValue)
          ? periodValue[0]
          : periodValue;
        ...
        if (periodValue) {
          ...
          // Convert to simple ISO date (YYYY-MM-DD)
          const date = new Date(cveperValue);
          if (!isNaN(date.getTime())) {
            cveperValue = date.toISOString().split("T")[0]; // "2025-06-29"
          }
        ...
        params.append("cveper", cveperValue);
        console.log(
          "📅 ✅ Agregando cveper en formato timestamp:",
          cveperValue
        );
      } else {
      ...
      if (!result.success || !result.data || result.data.length === 0) {
        console.warn(
          "❌ No se encontraron datos para el empleado con CURP:",
          curpValue
        );
        setPayrollData(null);
        setError(
          "No se encontraron datos para este empleado en el período seleccionado"
        );
        return;
      }
      ...
      console.log(
        "🗺️ Mapeo de períodos creado:",
        Object.fromEntries(nuevoMapeo)
      );
      ...
      result.data.forEach((registro, index) => {
        console.log(`📊 Registro ${index + 1}:`, {
          ...
          esElPeriodoCorrecto: periodValue
            ? registro.cveper === periodValue
            : "NO_FILTRADO_POR_PERIODO",
      ...
      const registrosConCURPCorrecta = result.data.filter(
        (r) => (r.curp || r.CURP) === curpValue
      );
      console.log(
        "- Registros con CURP correcta:",
        registrosConCURPCorrecta.length
      );
      ...
      if (periodValue) {
        ...
        if (formatoOriginalCompleto) {
          ...
          const registrosConPeriodoCorrecto = result.data.filter(
            (r) => r.cveper === formatoOriginalCompleto
          );
          console.log(
            "- Registros con período correcto (mapeado):",
            registrosConPeriodoCorrecto.length
          );

          const registrosConAmbosCorrectos = result.data.filter(
            (r) =>
              (r.curp || r.CURP) === curpValue &&
              r.cveper === formatoOriginalCompleto
          );
          console.log(
            "- Registros con CURP Y período correctos (mapeado):",
            registrosConAmbosCorrectos.length
          );
          ...
          if (registrosConAmbosCorrectos.length > 0) {
            ...
            console.log("🎯 Primer registro correcto:", {
              curp:
                registrosConAmbosCorrectos[0].curp ||
                registrosConAmbosCorrectos[0].CURP,
              cveper: registrosConAmbosCorrectos[0].cveper,
              nombre:
                registrosConAmbosCorrectos[0].nombre ||
                registrosConAmbosCorrectos[0]["Nombre completo"],
            });
          } else {
            console.log(
              "❌ No se encontraron registros con CURP y período específicos"
            );
          }
        } else {
          console.log(
            "❌ No se pudo mapear el período limpio al formato original"
          );
        }
      }
      ...
      console.log(
        "- Nombre:",
        empleadoData.nombre || empleadoData["Nombre completo"]
      );
      console.log(
        "- ¿Es la CURP correcta?",
        (empleadoData.curp || empleadoData.CURP) === curpValue
      );

      console.log("🎯 DATOS DEL REGISTRO SELECCIONADO:");
      console.log(
        "- CURP del registro:",
        empleadoData.curp || empleadoData.CURP
      );
      console.log("- CURP solicitada:", curpValue);
      console.log(
        "- ¿Coinciden?",
      ...
      console.log(
        "🔍 TODOS los campos disponibles del empleado:",
        Object.keys(empleadoData)
      );
      ...
      if (Object.keys(empleadoData).length <= 20) {
        console.log(
          "⚠️ PROBLEMA DETECTADO: El backend solo devuelve campos básicos"
        );
        console.log(
          "🔧 SOLUCIÓN NECESARIA: El backend debe usar SELECT * FROM historico_nominas_gsau"
        );
        console.log(
          '📋 CAMPOS ESPERADOS: Deberían haber 100+ campos con espacios como " SUELDO CLIENTE "'
        );
      } else {
      ...
      console.log(
        "🔍 VERIFICANDO CAMPOS ESPECÍFICOS QUE BUSCAN LAS SECCIONES:"
      );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <PercepcionesAdicionalesSection data={payrollData} loading={loading} />

      {/* Component D - Beneficios y Ajustes */}
      <BeneficiosAjustesSection data={payrollData} loading={loading} />

      {/* Component E - Totales y Costos */}
      <TotalesCostosSection data={payrollData} loading={loading} />

      {/* Component F - Deducciones */}
      <DeduccionesSection data={payrollData} loading={loading} />
    </div>
  );
};
