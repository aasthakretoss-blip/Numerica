    ...
    const cargarDatos = async () => {
      ...
      try {
        ...
        const response = await authenticatedFetch(
          `${buildApiUrl("/api/busqueda-empleados")}?${params.toString()}`
        );
        ...
        if (selectedCveper) {
          datosCompletos = registrosEmpleado.find(
            (registro) =>
              registro.cveper === selectedCveper ||
              registro.mes === selectedCveper
          );
        ...
        if (datosCompletos) {
          ...
          const camposPercepciones = Object.keys(datosCompletos).filter(
            (key) =>
          ...
          console.log(
            "💰 Campos relacionados con percepciones encontrados:",
            camposPercepciones
          );
          ...
          camposPercepciones.forEach((campo) => {
            console.log(
              `💰 ${campo}:`,
              datosCompletos[campo],
              typeof datosCompletos[campo]
            );
          });
          ...
          const camposNomina = Object.keys(datosCompletos).filter(
            (key) =>
  ...
  return (
    ...
    >
      <h3 style={{ margin: "0 0 1rem 0", color: "#007bff" }}>
        🔍 DEBUG - Análisis de Datos (RFC: {rfc})
      </h3>
      ...
      >
        <h4 style={{ margin: "0 0 0.5rem 0", color: "#28a745" }}>
          💰 PERCEPCIONES - Campos Específicos:
        </h4>
        <div
          style={{
            fontSize: "11px",
          ...
          {camposPercepciones.map((campo) => {
            ...
            return (
              ...
              >
                <strong>{campo}:</strong>
                <span>
                  {tieneValor ? `${valor} ✅` : `${valor || "undefined"} ❌`}
                </span>
              </div>
            );
          })}
      ...
      >
        <h4 style={{ margin: "0 0 0.5rem 0", color: "#856404" }}>
          ✅ TODOS LOS CAMPOS CON VALOR ({camposConValor.length}):
        </h4>
