  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        const fechasResponse = await fetch(`https://numerica-1.onrender.com/api/payroll/fecpla-from-rfc?rfc=${encodeURIComponent(rfc)}`);
        ...
        const dataPromises = fechasFPL.map(async (fechaFPL, index) => {
          ...
          try {
            const dataUrl = `https://numerica-1.onrender.com/api/fpl/data-from-rfc?rfc=${encodeURIComponent(rfc)}&originalFecpla=${encodeURIComponent(metadata.originalFecpla)}&originalAntiguedad=${encodeURIComponent(metadata.originalAntiguedad)}`;
