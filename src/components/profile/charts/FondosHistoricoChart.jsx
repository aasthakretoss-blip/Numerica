  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        const fechasResponse = await fetch(`https://ki6h36kbh4.execute-api.us-east-1.amazonaws.com/prod/api/payroll/fecpla-from-rfc?rfc=${encodeURIComponent(rfc)}`);
        ...
        const dataPromises = fechasFPL.map(async (fechaFPL, index) => {
          ...
          try {
            const dataUrl = `https://ki6h36kbh4.execute-api.us-east-1.amazonaws.com/prod/api/fpl/data-from-rfc?rfc=${encodeURIComponent(rfc)}&originalFecpla=${encodeURIComponent(metadata.originalFecpla)}&originalAntiguedad=${encodeURIComponent(metadata.originalAntiguedad)}`;
