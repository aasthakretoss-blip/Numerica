...
export default function TablaInteractivaDemografica({ 
  ...
  const loadDataFromSelection = async () => {
    if (!graphSelection || !graphSelection.data) {
      console.log('🎯 TablaInteractivaDemografica: No hay selección de gráfico');
      setEmployees([]);
    ...
    try {
      setLoading(true);
      console.log('🎯 TablaInteractivaDemografica: Cargando datos para selección:', graphSelection);
  ...
  useEffect(() => {
    if (graphSelection && graphSelection.data) {
      console.log('🎯 TablaInteractivaDemografica: Nueva selección detectada, recargando datos');
  ...
  const renderSelectionBadge = () => {
    ...
    switch (graphSelection.type) {
      ...
      case 'puesto_gender':
        const puestoGenderText = graphSelection.data.gender === 'male' ? 'Hombres' : 'Mujeres';
        return (
          <SelectionBadge>
      ...
      case 'salary_age_range':
        ...
        return (
          <SelectionBadge>
            Salario ${salaryRange[0].toLocaleString()}-${salaryRange[1].toLocaleString()}
          </SelectionBadge>
        );
      ...
      case 'sucursal_antiguedad':
        return (
          <SelectionBadge>
            {graphSelection.data.sucursal} - {graphSelection.data.antiguedadRange} años
          </SelectionBadge>
        );
      ...
      <TableContent $collapsed={collapsed}>
        {!graphSelection || !graphSelection.data ? (
          <EmptyState>
            ...
            <p>
              Haz clic en cualquier barra de los gráficos de arriba para ver los datos específicos aquí.
              <br />
              Puedes seleccionar en: Pirámide poblacional, Distribución por puestos, Rangos salariales, etc.
            </p>
          </EmptyState>
        ) : loading ? (
          <LoadingContainer>
            ...
            <TableWrapper>
              <Table>
                ...
                <TableBody>
                  ...
                  {transformedEmployees.length === 0 && !loading && graphSelection && graphSelection.data && (
                    <tr>
                      <TableCell colSpan={columns.length}>
                        <div style={{ textAlign: 'center', padding: '2rem', color: 'rgba(44, 62, 80, 0.7)' }}>
