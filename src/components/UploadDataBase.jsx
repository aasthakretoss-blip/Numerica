...
const Container = styled.div`
  padding: 2rem;
  background: ${(props) =>
    props.theme?.surfaces?.glass?.medium || surfaces.glass.medium};
  backdrop-filter: ${(props) =>
    props.theme?.effects?.blur?.strong || effects.blur.strong};
  border-radius: 20px;
  border: 1px solid
    ${(props) =>
      props.theme?.surfaces?.borders?.light || surfaces.borders.light};
  margin: 2rem;
  min-height: calc(100vh - 200px);
  transition: ${(props) =>
    props.theme?.effects?.states?.transition || effects.states.transition};
`;
...
const Title = styled.h2`
  ...
  text-align: center;
  background: ${(props) =>
    props.theme?.gradients?.text?.primary || gradients.text.primary};
...
const UploadSection = styled.div`
  background: ${(props) =>
    props.theme?.surfaces?.glass?.light || surfaces.glass.light};
  border: 2px dashed
    ${(props) =>
      props.theme?.surfaces?.borders?.medium || surfaces.borders.medium};
  border-radius: 15px;
  padding: 3rem 2rem;
  text-align: center;
  margin-bottom: 2rem;
  ...
  cursor: pointer;
  backdrop-filter: ${(props) =>
    props.theme?.effects?.blur?.light || effects.blur.light};

  &:hover {
    border-color: ${(props) =>
      props.theme?.surfaces?.borders?.accent || surfaces.borders.accent};
    background: ${(props) =>
      props.theme?.surfaces?.glass?.medium || surfaces.glass.medium};
    transform: ${(props) =>
      props.theme?.effects?.states?.hoverTransform ||
      effects.states.hoverTransform};
  }

  &.dragover {
    border-color: ${(props) =>
      props.theme?.surfaces?.borders?.accentStrong ||
      surfaces.borders.accentStrong};
    background: ${(props) =>
      props.theme?.surfaces?.glass?.strong || surfaces.glass.strong};
    box-shadow: ${(props) =>
      props.theme?.effects?.shadows?.medium || effects.shadows.medium};
  }
`;
...
const PathInput = styled.input`
  ...
  padding: 1rem;
  background: ${(props) =>
    props.theme?.surfaces?.inputs?.background || surfaces.inputs.background};
  border: 1px solid
    ${(props) =>
      props.theme?.surfaces?.inputs?.border || surfaces.inputs.border};
  border-radius: 8px;
  color: ${(props) => props.theme?.text?.primary || textColors.primary};
  font-size: 0.9rem;
  transition: ${(props) =>
  ...
  &:focus {
    ...
    border-color: ${(props) =>
      ...
      surfaces.borders.accentStrong};
    background: ${(props) =>
      props.theme?.surfaces?.inputs?.focus || surfaces.inputs.focus};
    box-shadow: ${(props) =>
      props.theme?.effects?.states?.focusRing || effects.states.focusRing};
  }
`;
...
const FileInfo = styled.div`
  ...
  border: 1px solid
    ${(props) =>
      props.theme?.surfaces?.borders?.subtle || surfaces.borders.subtle};
  backdrop-filter: ${(props) =>
    props.theme?.effects?.blur?.light || effects.blur.light};
  transition: ${(props) =>
    props.theme?.effects?.states?.transition || effects.states.transition};
...
const DetectedType = styled.div`
  ...
  background: ${(props) => {
    if (props.type === "nominas") {
      return props.theme?.semanticColors
        ? `rgba(${props.theme.semanticColors.success
            .slice(1)
            .match(/../g)
    ...
    if (props.type === "fondos") {
      return props.theme?.semanticColors
        ? `rgba(${props.theme.semanticColors.info
            .slice(1)
            .match(/../g)
            .map((hex) => parseInt(hex, 16))
            .join(", ")}, 0.2)`
    ...
    return props.theme?.semanticColors
      ? `rgba(${props.theme.semanticColors.warning
          .slice(1)
          .match(/../g)
          .map((hex) => parseInt(hex, 16))
          .join(", ")}, 0.2)`
...
const Button = styled.button`
  ...
  &:disabled {
    ...
    cursor: not-allowed;
    background: ${(props) =>
      props.theme?.surfaces?.buttons?.disabled || surfaces.buttons.disabled};
  }
`;

const UploadButton = styled(Button)`
  background: ${(props) =>
    props.theme?.gradients?.buttons?.primary || gradients.buttons.primary};
  color: ${(props) => props.theme?.text?.primary || textColors.primary};
  box-shadow: ${(props) =>
    props.theme?.effects?.shadows?.medium || effects.shadows.medium};
  ...
  &:hover:not(:disabled) {
    transform: ${(props) =>
      ...
      effects.states.hoverTransform};
    box-shadow: ${(props) =>
      props.theme?.effects?.shadows?.colored || effects.shadows.colored};
    opacity: 0.9;
  }
`;
...
const ClearButton = styled(Button)`
  background: ${(props) =>
    props.theme?.surfaces?.buttons?.secondary || surfaces.buttons.secondary};
  color: ${(props) => props.theme?.text?.primary || textColors.primary};
  border: 1px solid
    ${(props) =>
      props.theme?.surfaces?.borders?.medium || surfaces.borders.medium};
...
const StatusMessage = styled.div`
  ...
  &.success {
    background: ${(props) => {
      const successColor =
        props.theme?.semanticColors?.success || semanticColors.success;
  ...
  &.error {
    background: ${(props) => {
      const errorColor =
        props.theme?.semanticColors?.error || semanticColors.error;
  ...
  &.info {
    background: ${(props) => {
      const infoColor =
        props.theme?.semanticColors?.info || semanticColors.info;
...
const UploadDataBase = () => {
  ...
  const validateFile = async (file) => {
    ...
    try {
      ...
      const response = await authenticatedFetch("/api/validate-file", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        setStatusMessage({
          type: "success",
          message: `Archivo validado: Detectado como formato de ${
            result.type === "nominas" ? "Nóminas" : "Fondos"
          }`,
        });
      } else {
        setStatusMessage({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await authenticatedFetch("/api/upload-data", {
        method: "POST",
        body: formData,
      });
