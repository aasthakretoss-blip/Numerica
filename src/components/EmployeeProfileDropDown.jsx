...
const EmployeeProfileDropDown = ({
  ...
  const [usingEmployeeSpecificPeriods, setUsingEmployeeSpecificPeriods] =
    useState(false);
  ...
  const formatPeriodOption = (option) => {
    try {
      ...
      if (
        ...
        (dateValue.includes("-") || dateValue.includes("T"))
      ) {
      ...
      return {
        ...
        formattedLabel: `${displayValue}${
          option.count ? ` (${option.count.toLocaleString("es-MX")})` : ""
        }`,
      };
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const sourceOptions =
      usingEmployeeSpecificPeriods && employeePeriods.length > 0
        ? employeePeriods
    ...
    if (sourceOptions.length > 0) {
      console.log(
        "📅 EmployeeProfileDropDown: Procesando opciones de períodos:",
        {
          usando: usingEmployeeSpecificPeriods
            ? "Períodos específicos del empleado"
            : "Períodos generales",
      ...
      if (
        ...
        usingEmployeeSpecificPeriods
      ) {
        const mostRecent = menuOptions[0]; // Ya están ordenados por fecha descendente
        console.log(
          "🎯 EmployeeProfileDropDown: Auto-seleccionando período más reciente específico:",
          mostRecent
        );
  ...
  const loadEmployeePeriods = async (employeeCurp) => {
    ...
    setIsLoading(true);
    console.log(
      "🔍 Cargando períodos CVEPER específicos para empleado:",
      employeeCurp
    );
    ...
    try {
      ...
      console.log(
        "📅 Respuesta completa de períodos CVEPER desde historico_nominas_gsau:",
        {
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        const cveperField =
          Object.keys(firstRecord).find(
            (key) =>
              key.toLowerCase().includes("cveper") ||
              key.toLowerCase() === "cveper"
          ) || "cveper";
        ...
        const registrosDelEmpleado = result.data.filter((emp) => {
          ...
          const tieneCveper =
            emp[cveperField] != null &&
            emp[cveperField] !== "" &&
            emp[cveperField] !== undefined;
        ...
        const uniqueCveperValues = [
          ...new Set(registrosDelEmpleado.map((emp) => emp[cveperField])),
        ...
        const employeeOptions = uniqueCveperValues.map((cveper) => ({
          value: cveper,
          count: registrosDelEmpleado.filter(
            (emp) => emp[cveperField] === cveper
          ).length,
        }));

        console.log(
          '✅ Opciones finales para el dropdown "Dato de percepción":',
          {
        ...
        if (employeeOptions.length > 0) {
          ...
          console.warn(
            "⚠️ No se encontraron fechas CVEPER válidas para el CURP:",
            employeeCurp
          );
          setEmployeePeriods([]);
          setUsingEmployeeSpecificPeriods(false);
        ...
        console.log(
          "⚠️ No se encontraron registros CVEPER para el empleado, usando períodos generales como fallback"
        );
      ...
      console.error(
        "❌ Error cargando períodos CVEPER específicos del empleado:",
        error
      );
      setEmployeePeriods([]);
      setUsingEmployeeSpecificPeriods(false);
  ...
  const displaySelectedValues = useMemo(() => {
    return selectedValues.map((originalValue) => {
      const option = formattedOptions.find(
        (opt) => opt.originalValue === originalValue
      );
