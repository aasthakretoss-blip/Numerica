...
export default function SalaryAgePopulationPyramid({
  ...
  const [uniqueGenderCounts, setUniqueGenderCounts] = useState({
    male: 0,
    female: 0,
    unknown: 0,
    total: 0,
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data && result.data.length > 0) {
          ...
          console.log(
            "📅 SalaryAgePopulationPyramid - Último período encontrado:",
            latest.value,
            "Filtro aplicado:",
            monthFilter
          );
  ...
  const loadUniqueCurpCount = async () => {
    try {
      ...
      console.log(
        "🔍 SalaryAgePopulationPyramid: Contando CURPs únicos con filtros:",
        filterParams
      );
  ...
  const loadSalaryData = async () => {
    try {
      ...
      console.log(
        "📊 SalaryAgePopulationPyramid: Aplicando filtros demográficos:",
        {
          filterParams,
      ...
      console.log(
        "📊 SalaryAgePopulationPyramid: Cargando TODAS las páginas..."
      );
      ...
      do {
        ...
        console.log(
          `📄 SalaryAgePopulationPyramid: Cargando página ${currentPage}/${totalPages}...`
        );
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            if (currentPage === 1) {
              ...
              console.log(
                `📊 SalaryAgePopulationPyramid: Total esperado: ${expectedTotal}, Páginas: ${totalPages}`
              );
              ...
              if (result.data && result.data.length > 0) {
                ...
                console.log(
                  "🔍 DEBUG: Buscando campos CURP:",
                  Object.keys(result.data[0]).filter((key) =>
                    key.toLowerCase().includes("curp")
                  )
                );
                console.log(
                  "🔍 DEBUG: Buscando campos percepciones:",
                  Object.keys(result.data[0]).filter((key) =>
                    key.toLowerCase().includes("percepc")
                  )
                );

                const curpField = Object.keys(result.data[0]).find((key) =>
                  key.toLowerCase().includes("curp")
                );
                if (curpField) {
                  console.log(
                    "🔍 DEBUG: Campo CURP encontrado:",
                    curpField,
                    '= "' + result.data[0][curpField] + '"'
                  );
                }

                const percepcionesField = Object.keys(result.data[0]).find(
                  (key) => key.toLowerCase().includes("percepc")
                );
                if (percepcionesField) {
                  console.log(
                    "🔍 DEBUG: Campo percepciones encontrado:",
                    percepcionesField,
                    "=",
                    result.data[0][percepcionesField]
                  );
                }
              }
            }
            ...
            if (result.data && result.data.length > 0) {
              ...
              console.log(
                `✅ SalaryAgePopulationPyramid: Página ${currentPage} cargada - ${result.data.length} registros (Total acumulado: ${allEmployees.length})`
              );
            } else {
              console.log(
                `⚠️ SalaryAgePopulationPyramid: Página ${currentPage} vacía`
              );
              break;
            }
            ...
            console.error(
              `❌ SalaryAgePopulationPyramid: Error en página ${currentPage}:`,
              result.error
            );
            break;
          }
          ...
          console.error(
            `❌ SalaryAgePopulationPyramid: Error HTTP en página ${currentPage}:`,
            response.status,
            response.statusText
          );
          break;
        ...
        allEmployees.length < expectedTotal
      );
      ...
      console.error(
        "❌ SalaryAgePopulationPyramid: Error loading demographic data:",
        error
      );
    } finally {
      setLoading(false);
  ...
  const getSalaryBand = (costoNomina) => {
    ...
    return (
      SALARY_BANDS.find((band) => salary >= band.min && salary < band.max) ||
      SALARY_BANDS[0]
    );
  };

  // Función para manejar clics en las barras
  const handleBarClick = (age, gender, salaryBand) => {
    // Crear selección usando el nuevo sistema
    const selection = createSelection.salaryAgeGenderBand(
      age,
      gender,
      salaryBand,
      {
        totalEmployees: pyramidData.stats?.totalEmployees || 0,
        ageCount: pyramidData.ageGroups[age]?.[gender]?.[salaryBand.name] || 0,
        filters: filters,
    ...
    console.log(
      "🎯 SalaryAgePopulationPyramid: Emitiendo selección:",
      selection
    );
  ...
  const pyramidData = useMemo(() => {
    if (!salaryData.length)
      return {
        ageGroups: {},
        stats: {},
        uniqueGenderCounts: { male: 0, female: 0, unknown: 0, total: 0 },
      };
    ...
    salaryData.forEach((emp, index) => {
      ...
      const curpField = Object.keys(emp).find((key) =>
        key.toLowerCase().includes("curp")
      );
      ...
      const costoField = Object.keys(emp).find(
        (key) =>
      ...
      if (index < 5) {
        ...
        console.log(
          `  - Campos disponibles: [${Object.keys(emp)
            .slice(0, 10)
            .join(", ")}...]`
        );
        console.log("  ---");
    ...
    console.log(
      `  - CURPs únicos encontrados: ${Object.keys(employeesByCurp).length}`
    );
    console.log(
      `  - Campos CURP encontrados: [${Array.from(curpFieldsFound).join(", ")}]`
    );
    console.log(
      `  - Campos Costo encontrados: [${Array.from(costoFieldsFound).join(
        ", "
      )}]`
    );
    console.log("=".repeat(60));
    ...
    console.log(
      "👥 DEBUG: Empleados únicos por CURP:",
      Object.keys(employeesByCurp).length
    );
    console.log(
      "👥 DEBUG: Ejemplo de empleado agrupado:",
      Object.values(employeesByCurp)[0]
    );
    ...
    console.log(
      "  - ¿Suma correcta?",
      empleadosConSalarioPositivo + empleadosConSalarioCero ===
        Object.keys(employeesByCurp).length
    );
    ...
    if (ejemplosConSalarioCero.length > 0) {
      ...
      ejemplosConSalarioCero.forEach((ejemplo, i) => {
        console.log(
          `  ${i + 1}. CURP: ${ejemplo.curp}, Costo: $${
            ejemplo.costo
          }, Registros: ${ejemplo.registros}`
        );
      });
    }
    ...
    console.log(
      "  - Suma verificación:",
      uniqueGenderCounts.male +
        uniqueGenderCounts.female +
        uniqueGenderCounts.unknown
    );
    console.log(
      "  - ¿Suma correcta?",
      uniqueGenderCounts.male +
    ...
    console.log(
      "🎯 Usando datos reales procesados directamente (sin extrapolación):",
      {
    ...
    const avgAge =
      validEmployees.length > 0
        ? validEmployees.reduce((sum, { age }) => sum + age, 0) /
    ...
    console.log("👥 Distribución por género:", {
      mujeres: finalFemaleCount,
      hombres: finalMaleCount,
    });
    ...
    console.log(
      "  - ¿Coinciden?",
      finalMaleCount + finalFemaleCount === totalEmployees
    );
    console.log("  - Distribución por banda salarial:");
    SALARY_BANDS.forEach((band) => {
      const bandStats = salaryBandStats[band.name];
      console.log(
        `    ${band.name}: Total=${bandStats.count}, M=${bandStats.male}, F=${
          bandStats.female
        }, Suma=${bandStats.male + bandStats.female}`
      );
    });
    ...
    console.log(
      "  - TOTAL EXCLUIDOS:",
      invalidCurp + invalidAge + invalidSalary
    );

    console.log("\n🔢 COMPARACIÓN DE TOTALES:");
    console.log("  - Servidor (endpoint unique-count):", totalUniqueEmployees);
    console.log(
      "  - Calculado localmente (todos CURPs):",
      uniqueGenderCounts.total
    );
    console.log(
      "  - Diferencia:",
      Math.abs(totalUniqueEmployees - uniqueGenderCounts.total)
    );
  ...
  const {
    ...
    SALARY_BANDS.forEach((band) => {
      const maleMax = Math.max(
        ...Object.values(pyramidData.ageGroups).map(
          (ageData) => ageData.male?.[band.name] || 0
        ),
        0
      );
      const femaleMax = Math.max(
        ...Object.values(pyramidData.ageGroups).map(
          (ageData) => ageData.female?.[band.name] || 0
        ),
        0
      );
  ...
  const { ageRange, chartHeight, rowHeight } = useMemo(() => {
    ...
    const agesWithData = Object.keys(ageGroups)
      ...
      .filter((age) => {
        ...
        return SALARY_BANDS.some(
          (band) =>
            (data.female?.[band.name] || 0) > 0 ||
            (data.male?.[band.name] || 0) > 0
        );
      })
      .sort((a, b) => a - b);
          ...
          <PyramidWrapper>
            <LeftSidebar>
              <StatsContainer>
                <StatItem>
                  <StatValue>
                    {pyramidData.stats?.totalEmployees || 0}
                  </StatValue>
                ...
                <StatItem>
                  <StatValue>
                    $
                    {(pyramidData.stats?.averageSalary || 0).toLocaleString(
                      "es-MX",
                ...
                <StatItem>
                  <StatValue>
                    {(pyramidData.stats?.averageAge || 0).toFixed(1)}
                  </StatValue>
                  <StatLabel>Edad Promedio</StatLabel>
                </StatItem>
                <StatItem>
                  <StatValue>
                    {pyramidData.uniqueGenderCounts?.female || 0}
                  </StatValue>
                  <StatLabel>Mujeres</StatLabel>
                </StatItem>
                <StatItem>
                  <StatValue>
                    {pyramidData.uniqueGenderCounts?.male || 0}
                  </StatValue>
                ...
                {SALARY_BANDS.map((band) => (
                  <LegendItem key={band.name}>
                    ...
                    <span style={{ fontSize: "0.7rem", opacity: 0.7 }}>
                      (
                      {pyramidData.stats?.salaryBandStats?.[band.name]?.count ||
                        0}
                      )
                    </span>
                  </LegendItem>
              ...
              <PyramidChart $chartHeight={chartHeight}>
                {ageRange.length === 0 && (
                  ...
                  >
                    ...
                    <p style={{ fontSize: "0.8rem", opacity: 0.7 }}>
                      Grupos de edad:{" "}
                      {Object.keys(pyramidData.ageGroups || {}).length}
                    </p>
                  </div>
                )}
                {ageRange.map((age) => {
                  ...
                  const totalForAge = SALARY_BANDS.reduce(
                    (sum, band) =>
                      sum +
                      (femaleData[band.name] || 0) +
                      (maleData[band.name] || 0),
                    0
                  );
                  ...
                  return (
                    <AgeRow key={age} $rowHeight={rowHeight}>
                      ...
                      <LeftContainer
                        $maxWidth={`${containerWidthPercent / 2}%`}
                      >
                        {SALARY_BANDS.slice()
                          .reverse()
                          .map((band) => {
                            ...
                            const width =
                              globalMaxCount > 0
                                ? (count / globalMaxCount) * scaleFactor
                                : 0;
                            ...
                            return (
                              <SalaryBar
                                ...
                                onClick={() =>
                                  handleBarClick(age, "female", band)
                                }
                                title={`Clic para ver detalles: ${count} mujeres de ${age} años - ${band.label}`}
                              >
                                <TooltipBar>
                                  Edad: {age} años
                                  <br />
                        ...
                        {SALARY_BANDS.map((band) => {
                          ...
                          return (
                            <SalaryBar
                              ...
                              $gradient={band.color}
                              $opacity={0.8 + (count / globalMaxCount) * 0.2}
                              onClick={() => handleBarClick(age, "male", band)}
                              title={`Clic para ver detalles: ${count} hombres de ${age} años - ${band.label}`}
                            >
                              <TooltipBar>
                                Edad: {age} años
                                <br />
