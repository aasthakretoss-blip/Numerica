...
const ToggleButton = styled.button`
  ...
  &:hover {
    background: ${props => props.theme?.surfaces?.buttons?.accentStrong || 'rgba(30, 58, 138, 0.4)'};
...
const StatsPanel = styled.div`
  ...
  margin-top: 1.5rem;
  padding-top: 1.5rem;
  border-top: 1px solid ${props => props.theme?.surfaces?.borders?.subtle || 'rgba(255, 255, 255, 0.1)'};
`;
...
const EmptyState = styled.div`
  ...
  h4 {
    margin: 0 0 0.5rem 0;
    color: ${props => props.theme?.text?.secondary || 'rgba(44, 62, 80, 0.8)'};
  }
...
const EmployeeCompareGraph = ({ 
  ...
  const processedData = useMemo(() => {
    ...
    console.log('🔄 Procesando datos para gráfica:', {
      ...
      primeros5: employees.slice(0, 5).map(emp => ({
        ...
        total: emp[' TOTAL DE PERCEPCIONES '] || emp.percepcionesTotales || emp.totalPercepciones
      }))
    });
    ...
    employees.forEach((employee, index) => {
      ...
      if (groupedByEmployeeMonth.has(key)) {
        ...
        existing.total += (parseFloat(employee[' TOTAL DE PERCEPCIONES '] || employee.percepcionesTotales || employee.totalPercepciones) || 0);
        ...
        const newRecord = {
          name: name,
          curp: employee.curp || employee.curve || employee.name || `emp_${index}`,
          periodo: periodo,
          salary: parseFloat(employee.salary) || 0,
          commissions: parseFloat(employee.commissions) || 0,
          total: parseFloat(employee[' TOTAL DE PERCEPCIONES '] || employee.percepcionesTotales || employee.totalPercepciones) || 0,
          recordCount: 1,
    ...
    if (['percepcionesTotales', ' TOTAL DE PERCEPCIONES ', 'comisiones', 'salario'].includes(localSortBy)) {
    ...
    console.log('✅ Datos procesados para gráfica (agrupados por empleado+mes):', {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (context) => {
            ...
            return `${employee.name} - ${employee.periodo} (${employee.recordCount} registro${employee.recordCount > 1 ? 's' : ''})`;
          },
          label: (context) => {
  ...
  const statistics = useMemo(() => {
    ...
    const totalCommissions = processedData.reduce((sum, emp) => sum + emp.commissions, 0);
    const totalPerceptions = processedData.reduce((sum, emp) => sum + emp.total, 0);
      ...
      {statistics && !isCollapsed && (
        <StatsPanel>
          ...
          <StatItem>
            ...
            <div className="value">${statistics.avgSalary.toLocaleString('es-MX')}</div>
          </StatItem>
          <StatItem>
            <div className="label">Comisiones Promedio</div>
            <div className="value">${statistics.avgCommissions.toLocaleString('es-MX')}</div>
          </StatItem>
          <StatItem>
            <div className="label">Percepciones totales Promedio</div>
            <div className="value">${statistics.avgTotal.toLocaleString('es-MX')}</div>
          </StatItem>
        </StatsPanel>
      )}
    </GraphContainer>
