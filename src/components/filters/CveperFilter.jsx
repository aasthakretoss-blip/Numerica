...
const DropdownHeader = styled.div`
  ...
  border: 2px solid
    ${(props) =>
      props.theme?.surfaces?.borders?.subtle || "rgba(255, 255, 255, 0.15)"};
  border-radius: 12px;
...
const DropdownList = styled.div`
  ...
  right: 0;
  background: ${(props) =>
    props.theme?.surfaces?.glass?.strong || "rgba(255, 255, 255, 0.2)"};
  backdrop-filter: ${(props) =>
    props.theme?.effects?.blur?.strong || "blur(20px)"};
  ...
  &::-webkit-scrollbar-thumb {
    background: ${(props) =>
      props.theme?.surfaces?.borders?.subtle || "rgba(255, 255, 255, 0.3)"};
    border-radius: 3px;
  }
`;
  ...
  useEffect(() => {
    const fetchCveperOptions = async () => {
      ...
      try {
        ...
        const uniqueCvepers = [
          ...new Set(result.data.map((item) => item["Mes"])),
        ]
  ...
  const formatCveperDisplay = (cveper) => {
    ...
    try {
      ...
      if (
        ...
        monthNames.includes(cveper.toUpperCase())
      ) {
