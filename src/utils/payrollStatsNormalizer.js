...
export function normalizePayrollStats(apiResponse, apiUrl = 'unknown') {
  console.log(`[PAYROLL STATS NORMALIZER] Processing response from API: ${apiUrl}`);
  ...
  if (!apiResponse || !apiResponse.success) {
    console.log(`[PAYROLL STATS NORMALIZER] Response not successful, returning as-is`);
    return apiResponse;
  }

  // If old format (has data property), return directly
  if (apiResponse.data) {
    console.log(`[PAYROLL STATS NORMALIZER] ✅ Response already in old format (has 'data' property)`);
  ...
  console.log(`[PAYROLL STATS NORMALIZER] ⚠️ Response in new format (has 'stats' property), normalizing...`);
  ...
  const estimatedUniqueEmployees = totalRecords > 0 
  ...
  const averageRecordsPerEmployee = estimatedUniqueEmployees > 0
  ...
  console.log(`[PAYROLL STATS NORMALIZER] ⚠️ Note: totalFondosRecords is set to 0 (not available in new API format)`);
...
async function fetchTotalFondosRecords(statsApiUrl, fetchFn = fetch) {
  try {
    ...
    console.log(`[PAYROLL STATS NORMALIZER] Attempting to fetch totalFondosRecords from API`);
    ...
    for (const endpoint of possibleEndpoints) {
      try {
        ...
        if (fondosResponse.ok) {
          ...
          console.log(`[PAYROLL STATS NORMALIZER] Response from ${endpoint}:`, fondosJson);
          
          // Check various possible response formats
          const totalFondos = fondosJson.totalRecords || 
          ...
          if (totalFondos !== undefined && totalFondos !== null) {
            console.log(`[PAYROLL STATS NORMALIZER] ✅ Got totalFondosRecords from ${endpoint}: ${totalFondos}`);
            return parseInt(totalFondos) || 0;
          }
        } else {
          console.log(`[PAYROLL STATS NORMALIZER] Endpoint ${endpoint} returned status: ${fondosResponse.status}`);
        }
      } catch (e) {
        console.log(`[PAYROLL STATS NORMALIZER] Endpoint ${endpoint} not available:`, e.message);
        // Continue to next endpoint
      }
    }
    
    // If none of the endpoints work, return 0
    console.log(`[PAYROLL STATS NORMALIZER] ⚠️ Could not fetch totalFondosRecords from any API endpoint, using 0 as fallback`);
    console.log(`[PAYROLL STATS NORMALIZER] Tried endpoints:`, possibleEndpoints);
    return 0;
  } catch (error) {
    console.error(`[PAYROLL STATS NORMALIZER] Error fetching totalFondosRecords:`, error);
    return 0;
  }
}
...
export async function fetchPayrollStats(apiUrl, fetchFn = fetch) {
  ...
  console.log(`[PAYROLL STATS NORMALIZER] Timestamp: ${new Date().toISOString()}`);
  
  try {
    const response = await fetchFn(url);
    
    console.log(`[PAYROLL STATS NORMALIZER] API Response Status: ${response.status} ${response.statusText}`);
    
    if (!response.ok) {
      console.error(`[PAYROLL STATS NORMALIZER] ❌ API request failed: HTTP ${response.status}`);
      return {
        success: false,
    ...
    if (!json.success) {
      console.log(`[PAYROLL STATS NORMALIZER] Response indicates failure, returning as-is`);
      return json;
    }
    ...
    if (json.data && json.data.totalFondosRecords !== undefined) {
      ...
      console.log(`[PAYROLL STATS NORMALIZER] ✅ totalFondosRecords found in old format: ${totalFondosRecords}`);
    } else {
      // Try to fetch totalFondosRecords from API
      console.log(`[PAYROLL STATS NORMALIZER] Fetching totalFondosRecords from API...`);
    ...
    if (totalFondosRecords > 0) {
      ...
      console.log(`[PAYROLL STATS NORMALIZER] ✅ Updated totalFondosRecords from API: ${totalFondosRecords}`);
    }
    
    console.log(`[PAYROLL STATS NORMALIZER] Final normalized response:`, normalized);
    console.log(`[PAYROLL STATS NORMALIZER] ==========================================`);
    
    return normalized;
  } catch (error) {
    console.error(`[PAYROLL STATS NORMALIZER] ❌ Error fetching payroll stats:`, error);
