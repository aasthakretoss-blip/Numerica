...
export function useServerPagination(
  ...
  useEffect(() => {
    isDisabledRef.current =
      endpoint === "__DISABLED__" || !endpoint || endpoint.trim() === "";
    console.log("🔍 [useServerPagination.js] Endpoint changed:", {
      endpoint,
  ...
  const fetchData = async (
    ...
    if (
      ...
      console.log(
        "⏭️ [useServerPagination.js fetchData] Skipping fetch - endpoint is disabled:",
        endpoint,
        "isDisabledRef:",
        isDisabledRef.current
      );
    ...
    try {
      ...
      if (
        sortByParam === "percepcionesTotales" ||
        sortByParam === "totalPercepciones"
      ) {
        console.log(
          "💰 useServerPagination - FETCH with PERCEPCIONES TOTALES:",
          {
            sortBy: sortByParam,
            sortDir: sortDirParam,
            page,
      ...
      if (items.length > 0) {
        console.log(
          "🟢 First 10 items from API (BEFORE any frontend processing):"
        );
        items.slice(0, 10).forEach((item, idx) => {
          const totalPercepciones =
            item.totalPercepciones ||
            item[" TOTAL DE PERCEPCIONES "] ||
            item["totalPercepciones"];
          ...
          console.log(
            `      totalPercepciones: ${totalPercepciones} (type: ${typeof totalPercepciones}, isZero: ${
              totalPercepciones === 0 || totalPercepciones === "0"
            })`
          );
          console.log(`      sueldo: ${sueldo} (type: ${typeof sueldo})`);
          console.log(
            `      comisiones: ${comisiones} (type: ${typeof comisiones})`
          );
          console.log(`      All keys: ${Object.keys(item).join(", ")}`);
          console.log(
            `      Has 'totalPercepciones': ${"totalPercepciones" in item}`
          );
          console.log(
            `      Has ' TOTAL DE PERCEPCIONES ': ${
              " TOTAL DE PERCEPCIONES " in item
            }`
          );
        });
      } else {
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "⏭️ [useServerPagination.js useEffect] Skipping fetch - endpoint is disabled:",
        endpoint,
        "isDisabledRef:",
        isDisabledRef.current
      );
    ...
    console.log(
      "🔄 [useServerPagination.js useEffect] Fetching data with endpoint:",
      endpoint
    );
  ...
  const refresh = () => {
    ...
    if (
      ...
      console.log(
        "⏭️ [useServerPagination.js] Skipping refresh - endpoint is disabled (props mode)"
      );
      return;
    }
    fetchData(pagination.page, pagination.pageSize, sortBy, sortDir);
  ...
  const handleSortChange = (newSortBy, newSortDir) => {
    console.log("📊 useServerPagination: Sort change received:", {
      newSortBy,
      newSortDir,
    });
    ...
    // LOG ESPECIAL PARA PERCEPCIONES TOTALES
    if (
      newSortBy === "percepcionesTotales" ||
      newSortBy === "totalPercepciones"
    ) {
