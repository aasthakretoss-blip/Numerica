...
export const loadDemographicFilterCounts = async (activeFilters = {}) => {
  try {
    console.log('🔄 Recargando conteos dinámicos para demográfico:', activeFilters);

    const params = new URLSearchParams();
    
    // Aplicar filtros activos
    if (activeFilters.sucursales && activeFilters.sucursales.length > 0) {
      activeFilters.sucursales.forEach(sucursal => params.append('sucursal', sucursal));
    }
    
    if (activeFilters.puestos && activeFilters.puestos.length > 0) {
      activeFilters.puestos.forEach(puesto => params.append('puesto', puesto));
    }
    
    if (activeFilters.puestosCategorias && activeFilters.puestosCategorias.length > 0) {
      activeFilters.puestosCategorias.forEach(categoria => params.append('puestoCategoria', categoria));
    }
    ...
    const response = await fetch(buildApiUrl(`/api/payroll/filter-options?${params.toString()}`));
...
export const calculateLatestPeriodFromDatabase = async () => {
  try {
    ...
    const response = await fetch(buildApiUrl(`/api/payroll?${params.toString()}`));
    ...
    if (result.success && result.data && result.data.length > 0) {
      ...
      } else if (typeof latestCveper === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(latestCveper)) {
      ...
      const periodInfo = {
        ...
        label: `${periodDate.toLocaleDateString('es-MX', { month: 'long', year: 'numeric' })}`,
      ...
      console.warn('⚠️ No se encontraron datos para calcular el último período');
 ...
 */
export const buildDemographicFilterParams = (filters, additionalParams = {}) => {
  const params = new URLSearchParams();
  
  // Aplicar filtros de dropdown
  if (filters.sucursales && filters.sucursales.length > 0) {
    filters.sucursales.forEach(sucursal => params.append('sucursal', sucursal));
  }
  ...
  if (filters.puestosCategorias && filters.puestosCategorias.length > 0) {
    filters.puestosCategorias.forEach(categoria => params.append('puestoCategoria', categoria));
  }
