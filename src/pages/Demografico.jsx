...
const Demografico = () => {
  ...
  const loadLatestPeriod = async () => {
    try {
      console.log(
        "📅 Demografico: Calculando último período desde la base de datos..."
      );
      ...
      if (latestPeriodInfo) {
        console.log(
          "📅 Demografico - Último período calculado:",
          latestPeriodInfo
        );
      ...
      console.log(
        "🆘 Error en cálculo de período, probando sin filtro de período..."
      );
  ...
  const testAndSetBestPeriodFormat = async (originalPeriod) => {
    ...
    const formatsToTest = [
      ...
      `${periodDate.getFullYear()}-${String(periodDate.getMonth() + 1).padStart(
        2,
        "0"
      )}`, // YYYY-MM calculado
      ...
      try {
        ...
        if (testResponse.ok) {
          ...
          if (
            ...
            console.log(
              `✅ Formato "${format}" funciona: ${testResult.data.length} empleados encontrados`
            );
            setPeriodFilter(format);
            return;
          } else {
            console.log(
              `⚠️ Formato "${format}" no devuelve datos:`,
              testResult.data?.length || 0,
              "empleados"
            );
          }
          ...
          console.log(
            `❌ Formato "${format}" devuelve error HTTP:`,
            testResponse.status
          );
        }
      } catch (error) {
    ...
    console.warn(
      "⚠️ Ningún formato devolvió datos, probando sin filtro de período..."
    );
    ...
    console.warn(
      "⚠️ Como último recurso, usando formato original:",
      originalPeriod
    );
    ...
    try {
      ...
      if (testResponse.ok) {
        ...
        if (
          ...
          console.log(
            "📝 Nota: Se mostrarán TODOS los empleados activos (sin filtro de período)"
          );
        ...
        console.error(
          "❌ Error HTTP probando sin filtro:",
          testResponse.status
        );
        return false;
      }
  ...
  const loadUniqueEmployeeCount = async () => {
    try {
      ...
      const response = await authenticatedFetch(
        buildApiUrl(`/api/payroll/demographic/unique-count?${params}`)
      );
      if (response.ok) {
  ...
  const loadActiveEmployees = async () => {
    try {
      ...
      console.log(
        "📊 Demografico - Iniciando carga paginada de TODOS los empleados"
      );
      ...
      console.log(
        "📊 Estrategia paginada - Páginas necesarias:",
        totalPages,
        "Tamaño por página:",
        PAGE_SIZE
      );
      ...
      pageResults.forEach((result, index) => {
        if (result.data && result.data.length > 0) {
          ...
          console.log(
            `📊 Página ${result.page}: ${result.data.length} empleados (Total acumulado: ${totalLoaded})`
          );
        } else {
      ...
      console.log(
        "📊 CARGA COMPLETA - Empleados cargados:",
        allEmployees.length,
        "de",
        realTotalCount,
        "esperados"
      ...
      if (allEmployees.length > 0) {
        ...
        const processedEmployees = allEmployees.map((emp) => {
          ...
          if (curp && curp.length >= 11) {
            ...
            console.log(
              `🔍 CURP ${curp}: Carácter género=${genderChar}, Resultado=${gender}`
            );
          } else {
            console.warn(
              `⚠️ Empleado sin CURP válida:`,
        ...
        console.log(
          "✅ Demografico - Usando datos completos de todas las páginas"
        );

        if (actualCount > 0) {
          console.log(
            "👷 Demografico - Primer empleado (ejemplo):",
            allEmployees[0]
          );
        }
  ...
  const handleFiltersChange = useCallback((filters) => {
    ...
    setDemographicFilters((prevFilters) => {
      ...
      if (hasChanged) {
        console.log("✅ Demografico - Aplicando cambios de filtros:", {
          prevFilters,
          filters,
        });
        return filters;
      ...
      console.log(
        "⏭️ Demografico - Sin cambios en filtros, saltando actualización"
      );
      return prevFilters;
    });
  }, []);
