...
async function verificarSistema() {
  ...
  try {
    ...
    if (basicData.success) {
      console.log(
        `✅ Endpoint básico funciona - ${basicData.data.length} registros obtenidos`
      );
    ...
    if (searchData.success) {
      console.log(
        `✅ Búsqueda por nombre funciona - ${searchData.data.length} registros encontrados`
      );
    ...
    if (puestoData.success) {
      console.log(
        `✅ Filtro por puesto funciona - ${puestoData.data.length} asesores encontrados`
      );
    } else {
    ...
    if (estadoData.success) {
      console.log(
        `✅ Filtro por estado funciona - ${estadoData.data.length} empleados activos`
      );
    } else {
    ...
    const combinadoResponse = await fetch(
      `${API_BASE}/api/payroll?puesto=TECNICO&status=A`
    );
    const combinadoData = await combinadoResponse.json();

    if (combinadoData.success) {
      console.log(
        `✅ Filtros combinados funcionan - ${combinadoData.data.length} técnicos activos`
      );
    } else {
    ...
    console.log(
      "🔧 API disponible en: http://numericaapi.kretosstechnology.com:3001"
    );
  } catch (error) {
