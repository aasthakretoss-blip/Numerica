...
class GoogleDriveService {
  ...
  async initialize() {
    try {
      ...
      this.auth = new google.auth.GoogleAuth({
        keyFile: path.join(__dirname, '../config/google-service-account.json'), // You'll need to create this
      ...
      try {
        ...
        console.error('❌ Error en configuración alternativa:', fallbackError.message);
        return false;
      }
    }
  }
  ...
  async searchFilesByName(employeeName) {
    try {
      ...
      console.log(`📁 Encontrados ${files.length} archivos para ${employeeName}`);
      ...
      console.log('🔄 Retornando datos de prueba para desarrollo con PDF visible...');
      return {
        success: true,
        files: [{
          id: 'sample-pdf-demo',
          name: `CONSENTIMIENTO_${employeeName.replace(/\s+/g, '_').toUpperCase()}.pdf`,
          size: '1024000',
          modifiedTime: new Date().toISOString(),
          viewLink: 'https://mozilla.github.io/pdf.js/web/compressed.tracemonkey-pldi-09.pdf',
          downloadLink: 'https://mozilla.github.io/pdf.js/web/compressed.tracemonkey-pldi-09.pdf',
          isMockData: true
        }]
      };
    }
      ...
      try {
        ...
        if (result.success && result.files.length > 0) {
          ...
          const newFiles = result.files.filter(file => 
            !allFiles.some(existing => existing.id === file.id)
          );
          allFiles = allFiles.concat(newFiles);
        }
      } catch (error) {
        console.log(`⚠️ Error buscando con variación "${variation}":`, error.message);
      }
    }
