...
class NominasService {
  ...
  async getPayrollData(options = {}) {
    try {
      ...
      console.log('🔍 [getPayrollData] Iniciando consulta con options:', JSON.stringify(options, null, 2));
      ...
      if (options.orderBy) {
        ...
        if (options.orderBy === 'percepcionesTotales' || options.orderBy === 'totalPercepciones') {
          console.log('💰 Backend PERCEPCIONES TOTALES detected:', {
        ...
        const fieldMapping = {
          ...
          'comisiones': '(COALESCE(" COMISIONES CLIENTE ", 0) + COALESCE(" COMISIONES FACTURADAS ", 0))',
      ...
      // Ejecutar consultas
      console.log('🔍 [getPayrollData] Ejecutando query SQL:', query);
      console.log('🔍 [getPayrollData] Parámetros:', finalParams);
      ...
      console.log(`✅ [getPayrollData] Query ejecutada: ${dataResult.rows.length} registros obtenidos`);
      
      // Log de estructura del primer registro para debugging
      if (dataResult.rows.length > 0) {
        const firstRow = dataResult.rows[0];
        console.log('🔍 [getPayrollData] Primer registro - Campos:', Object.keys(firstRow));
        console.log('🔍 [getPayrollData] Primer registro - Valores:', {
          nombre: firstRow.nombre,
          sueldo: firstRow.sueldo,
          comisiones: firstRow.comisiones,
          totalpercepciones: firstRow.totalpercepciones,
  ...
  async getDatasetStats() {
    try {
      ...
      // 2. CURPs únicos (empleados únicos) - ACTUALIZADO para usar CURP (con mayúsculas)
      const uniqueCurpQuery = `
        SELECT COUNT(DISTINCT "CURP") as unique_curps 
        FROM historico_nominas_gsau 
        WHERE "CURP" IS NOT NULL AND "CURP" != ''
      `;
      const uniqueCurpResult = await client.query(uniqueCurpQuery);
      const uniqueCurps = parseInt(uniqueCurpResult.rows[0].unique_curps);
      
      // 3. Primer dato cronológico existente en cveper (Datos desde:)
      ...
      const stats = {
        totalRecords,
        uniqueEmployees: uniqueCurps, // Ahora basado en CURPs
        earliestPeriod, // Nuevo: primer dato cronológico
        uniquePeriods: 0, // No solicitado, mantenido para compatibilidad
        averageRecordsPerEmployee: uniqueCurps > 0 ? Math.round(totalRecords / uniqueCurps) : 0
      };
      
      console.log(`✅ Estadísticas AWS Historic: ${totalRecords.toLocaleString('es-MX')} registros, ${uniqueCurps.toLocaleString('es-MX')} CURPs únicos, desde: ${earliestPeriod}`);
