...
async def test_endpoint():
    ...
    return {'status': 'ok', 'message': 'API is working!', 'service': 'Numerica Payroll API'}
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute('SELECT COUNT(*) FROM employees LIMIT 1')
                count = cur.fetchone()[0]
                return {'status': 'ok', 'message': 'Database connected!', 'employee_count': count}
    except Exception as e:
